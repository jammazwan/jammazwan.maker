...
public class InputRouteBuilder extends RouteBuilder {
	...
	public void configure() {
		...
		from("file:src/main/resources/velocity?noop=true&delete=false&fileName=README.txt.vm")
				.process(velocityProcessor).to("velocity://velocity/README.txt.vm")
		...
		from("file:src/main/resources/velocity?noop=true&delete=false&fileName=pom.xml.vm")
				.process(velocityProcessor).to("velocity://velocity/pom.xml.vm").process(getProcessor(rootDir))
				.to(getFolder());

		from("file:src/main/resources/velocity?noop=true&delete=false&fileName=project.vm")
				.process(velocityProcessor).to("velocity://velocity/project.vm")
				.process(getProcessor(rootDir + ".")).to(getFolder());
		...
		from("file:src/main/resources/velocity?noop=true&delete=false&fileName=Test.java.vm")
				.process(velocityProcessor).to("velocity://velocity/Test.java.vm")
		...
		if(isStandalone){
			...
			from("file:src/main/resources/velocity?noop=true&delete=false&fileName=gitinit.sh.vm")
					.process(velocityProcessor).to("velocity://velocity/gitinit.sh.vm")
					.process(getProcessor(rootDir)).to(getFolder());

			from("file:src/main/resources/velocity?noop=true&delete=false&fileName=commit.sh.vm")
					.process(velocityProcessor).to("velocity://velocity/commit.sh.vm")
					.process(getProcessor(rootDir)).to(getFolder());

			from("file:src/main/resources/velocity?noop=true&delete=false&fileName=README.md.vm")
					.process(velocityProcessor).to("velocity://velocity/README.md.vm")
					.process(getProcessor(rootDir)).to(getFolder());
		}
	}
	...
	private Processor getProcessor(final String prefix) {
		return new Processor() {
			public void process(Exchange exchange) throws Exception {
				...
				exchange.getIn().setHeader("CamelFileName", constant(prefix + getFileName(replacements.get("xyzCode"), fileName)));
			}
		};
	}
